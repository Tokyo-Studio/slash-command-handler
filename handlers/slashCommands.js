		...
		client.on("ready", async () => {
			...
			if(config.loadSlashsGlobal == true) {
				...
				try {
					...
					console.log(`${`Successfully`.bold.green} Loaded ${`(/)`.brightGreen} ${allCommands.length} Commands For: ${`ALL SERVERS`.brightRed}`);
				} catch (error) {
					console.log(`${`Unable`.bold.red} To Load ${`(/)`.brightGreen} ${allCommands.length} Commands For: ${`ALL SERVERS`.brightRed}`);
				}
			}
		})
